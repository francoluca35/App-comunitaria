...
import { matchesPostSearch } from '@/lib/community-search'
...
export default function CategoriaPage() {
  ...
  return (
    <DashboardLayout>
      <CategoriaPageInner category={category} />
    </DashboardLayout>
  )
}
...
function CategoriaPageInner({ category }: { category: Category | 'all' }) {
  ...
  const filteredPosts = useMemo(() => {
    ...
    const combined = [...myPendingInCategory, ...visibleApproved].sort(
      (a, b) => b.createdAt.getTime() - a.createdAt.getTime()
    )
    return combined.filter((p) => matchesPostSearch(p, searchQuery))
  }, [posts, currentUser, category, searchQuery])
